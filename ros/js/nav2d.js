...
NAV2D.Navigator = function(options) {
  ...
  function sendGoal(pose) {
    ...
    // 数据存到站点中
    const goalMessage = goal.getMessage()
    window.__station.addStation(goalMessage)

    // create a marker for the goal2
    var goalMarker = new ROS2D.NavigationArrow2({
      size : 8,
      strokeSize : 1,
      fillColor : '#ff0000',
      strokeColor : '#F6AB00',
      pulse : true,
      id: window.__station.id
    });
  ...
  if (withOrientation === false){
    ...
    this.rootObject.addEventListener('stagemousedown', function(event) {
      console.log(event.relatedTarget === window.robotMarker)
      if (event.relatedTarget === window.robotMarker) { return false }
