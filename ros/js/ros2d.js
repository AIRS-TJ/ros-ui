...
ROS2D.NavigationArrow = function(options) {
  ...
  // draw the arrow
  var graphics = new window.createjs.Graphics();
  // line width
  graphics.setStrokeStyle(strokeSize);
  graphics.beginStroke(strokeColor);
  graphics.beginFill(fillColor);
  ...
  graphics.setStrokeStyle(strokeSize / 4);
  // graphics.moveTo(strokeSize / 4, 0)
  graphics.beginStroke("#503183").beginFill("#503183")// beginFill('#503183');

  const directionWidth = size / 3
  graphics.drawCircle(size + directionWidth, 0, directionWidth);
...
ROS2D.NavigationArrow2 = function(options) {
  ...
  var text = new createjs.Text(id+'', "0.05px", strokeColor);
  text.alpha = 0 
  text.textAlign = 'center'
  window.ros.viewer.scene.addChild(text)
  setTimeout(() => {
...
ROS2D.NavigationArrow2.prototype.__proto__ = createjs.Shape.prototype;
 ...
 */
 ROS2D.Viewer.prototype.resize = function(width, height) {
  ...
  // save scene scaling
  this.scene.scaleX = this.scene.scaleX * this.width / width;
  this.scene.scaleY = this.scene.scaleY * this.height / height;
};
